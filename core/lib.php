			...
			switch ( $kind ) {
				case 'version':
					...
					if ( version_compare( ITSEC_Core::get_plugin_version(), $version, '<' ) ) {
						$error->add(
							'version',
							sprintf( __( 'You must be running at least version %s of Solid Security.', 'it-l10n-ithemes-security-pro' ), $version )
						);
					}
				...
				case 'ssl':
					if ( $requirement !== is_ssl() ) {
						$error->add(
							'ssl',
							$requirement ? __( 'Your site must support SSL.', 'it-l10n-ithemes-security-pro' ) : __( 'Your site must not support SSL.', 'it-l10n-ithemes-security-pro' )
						);
					}
					break;
				case 'feature-flags':
					foreach ( $requirement as $flag ) {
						if ( ! ITSEC_Lib_Feature_Flags::is_enabled( $flag ) ) {
							$error->add(
								'feature-flags',
								sprintf(
									__( 'The \'%s\' feature flag must be enabled.', 'it-l10n-ithemes-security-pro' ),
				...
				case 'multisite':
					if ( $requirement === 'enabled' && ! is_multisite() ) {
						$error->add(
							'multisite',
							__( 'Multisite must be enabled.', 'it-l10n-ithemes-security-pro' )
						);
					} elseif ( $requirement === 'disabled' && is_multisite() ) {
						$error->add(
							'multisite',
							__( 'Multisite is not supported.', 'it-l10n-ithemes-security-pro' )
						);
					}
					break;
				case 'server':
					...
					if ( isset( $requirement['php'] ) && version_compare( $info['server']['php'], $requirement['php'], '<' ) ) {
						$error->add( 'server', sprintf( __( 'You must be running PHP version %s or later.', 'it-l10n-ithemes-security-pro' ), $requirement['php'] ) );
					}
					...
					if ( $missing ) {
						if ( count( $missing ) === 1 ) {
							$message = sprintf( __( 'The %s PHP extension is required.', 'it-l10n-ithemes-security-pro' ), ITSEC_Lib::first( $missing ) );
						} else {
							$message = wp_sprintf(
								_n(
				...
				case 'ip':
					if ( ! ITSEC_Lib_IP_Detector::is_configured() ) {
						$error->add( 'ip', __( 'You must select an IP Detection method in Global Settings.', 'it-l10n-ithemes-security-pro' ) );
					}
					break;
			}
		}
