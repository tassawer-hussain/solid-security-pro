...
final class ITSEC_Mail {
	...
	public function add_footer( $add_divider = true ) {
		...
		if ( ! ITSEC_Core::is_pro() ) {
			...
			$replacements = array(
				'two_factor'        => esc_html__( 'Want two-factor authentication, scheduled site scanning, ticketed support and more?', 'it-l10n-ithemes-security-pro' ),
				'get_pro'           => esc_html__( 'Get Solid Security Pro', 'it-l10n-ithemes-security-pro' ),
				'go_pro_link'       => $security_link,
				'why_pro'           => esc_html__( 'Why Go Pro', 'it-l10n-ithemes-security-pro' ),
		...
		$replacements = array(
			'security_resources'     => esc_html__( 'More Website Security Resources', 'it-l10n-ithemes-security-pro' ),
			'articles'               => esc_html__( 'WordPress Security News', 'it-l10n-ithemes-security-pro' ),
			'articles_link'          => $articles_link,
			'articles_content'       =>
				sprintf(
					esc_html__(
						'Be the first to get the latest WordPress security news, tips, and updates on the %1$sSolidWP Blog%3$s, including the %2$sWeekly WordPress Vulnerability Report.%3$s', 'it-l10n-ithemes-security-pro'
					),
			...
			'tutorials_content'      => esc_html__( 'Vulnerable WordPress plugins and themes are the #1 reason WordPress sites get hacked. Either quickly update the vulnerable theme, plugin or WordPress version immediately to the newest version or immediately deactivate the plugin or theme until a fix is available.', 'it-l10n-ithemes-security-pro' ),
			...
			'documentation_content'  =>
				sprintf(
					esc_html__(
						'Read Solid Security documentation and Frequently Asked Questions on the %1$sSolidWP Help Center.%2$s', 'it-l10n-ithemes-security-pro'
			...
			'support_content'        =>
				sprintf(
					esc_html__(
						'Pro customers have the best support team available as their security team. Contact the %1$s SolidWP Help Desk%2$s for help when you need answers.', 'it-l10n-ithemes-security-pro'
			...
			'unsubscribe_link_text'  =>
				esc_html__( 'This email was generated by the Solid Security plugin.', 'it-l10n-ithemes-security-pro' ) . '<br>' .
				sprintf(
					esc_html__(
						'To unsubscribe from these updates, visit the %1$sSettings page%2$s in the Solid Security plugin menu.', 'it-l10n-ithemes-security-pro'
			...
			'pro'                    => esc_html__( 'Pro Feature', 'it-l10n-ithemes-security-pro' ),
			'security_guide'         => esc_html__( 'Check Out Our Free WordPress Security Guide', 'it-l10n-ithemes-security-pro' ),
			'security_guide_content' => sprintf(
				esc_html__(
					'Learn simple WordPress security tips — including 3 kinds of security your site needs and 4 best security practices for keeping your WordPress site safe with our %1$sfree guide.%2$s', 'it-l10n-ithemes-security-pro'
	...
	public function add_user_footer() {
		...
		$link_text = sprintf( esc_html__( 'This email was generated by the Solid Security plugin on behalf of %s.', 'it-l10n-ithemes-security-pro' ), get_bloginfo( 'name', 'display' ) ) . '<br>';
		$link_text .= sprintf(
	...
	public function add_lockouts_table( $lockouts ) {
		...
		foreach ( $lockouts as $lockout ) {
			if ( 'user' === $lockout['type'] ) {
				...
				/* translators: 1: Username */
				$lockout['description'] = sprintf( wp_kses( __( '<b>Username:</b> %1$s', 'it-l10n-ithemes-security-pro' ), array( 'b' => array() ) ), $lockout['id'] );
			} else {
				/* translators: 1: IP address */
				$lockout['description'] = sprintf( wp_kses( __( '<b>IP:</b> %1$s', 'it-l10n-ithemes-security-pro' ), array( 'b' => array() ) ), $lockout['id'] );
			}
		...
		$replacements = array(
			'heading_types'  => __( 'IP/User', 'it-l10n-ithemes-security-pro' ),
			'heading_until'  => __( 'Locked Out Until', 'it-l10n-ithemes-security-pro' ),
	...
	public function get_site_scanner_pro_callout() {
		...
		$template = $this->replace_all( $template, array(
			'title'             => esc_html__( 'Go Pro Now to Get Automatic Vulnerability Patching', 'it-l10n-ithemes-security-pro' ),
			'content'           =>
				wp_kses(
					__( 'Solid Security Pro will <b>automatically update vulnerable plugins and themes for you</b> if a patch is available <b>so you don’t have to manually log in to update</b>.', 'it-l10n-ithemes-security-pro' ),
					'mail'
				) . ' ' .
				esc_html__( 'Get your site patched by Solid Security Pro before hackers discover vulnerabilities on your site, all without doing a thing.', 'it-l10n-ithemes-security-pro' ) . '<br><br>' .
				sprintf(
					'<b>%s</b>',
					sprintf(
						esc_html__( 'Get all of this in the %1$sSite Scanner Pro%2$s:', 'it-l10n-ithemes-security-pro' ),
						'<a href="' . ITSEC_Core::get_tracking_link( 'https://go.solidwp.com/pro-feature-site-scan', 'sitescanemail', 'link' ) . '">',
						'</a>'
					)
				),
			'first_box_header'  => esc_html__( 'Save Time', 'it-l10n-ithemes-security-pro' ),
			'first_box'         => esc_html__( 'Save time when time is of the utmost importance to patch vulnerabilities before hackers and bots can find and exploit them.', 'it-l10n-ithemes-security-pro' ),
			'second_box_header' => esc_html__( 'More Freedom', 'it-l10n-ithemes-security-pro' ),
			'second_box'        => esc_html__( 'Free your team from mundane updates by removing the need to manually log in to update plugins and themes.', 'it-l10n-ithemes-security-pro' ),
			'third_box_header'  => esc_html__( 'Automatic Updates', 'it-l10n-ithemes-security-pro' ),
			'third_box'         => esc_html__( 'Automatically updates vulnerable plugins, themes, and WordPress core for you if it fixes a vulnerability that was found by the Site Scanner.', 'it-l10n-ithemes-security-pro' ),
			'fourth_box_header' => esc_html__( 'Increase Security', 'it-l10n-ithemes-security-pro' ),
			'fourth_box'        => esc_html__( 'Hardens your website if you are running outdated software, including checks for old WordPress sites that could be used to compromise your server.', 'it-l10n-ithemes-security-pro' ),
			'href'              => ITSEC_Core::get_tracking_link( 'https://go.solidwp.com/footer-site-scan', 'sitescanemail', 'button' ),
			'link_text'         => __( 'Go Pro Now', 'it-l10n-ithemes-security-pro' ),
			'bk_color'          => '#772ecb',
			'txt_color'         => '#FFFFFF',
		) );
	...
	public function set_default_subject() {
		return __( 'New Notification from Solid Security', 'it-l10n-ithemes-security-pro' );
	}
