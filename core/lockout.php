...
final class ITSEC_Lockout {
	...
	public function can_create_ban_from_lockout( int $lockout_id ): Result {
		if ( ! $this->is_lockout_banning_available() ) {
			return Result::error( new WP_Error(
				...
				__( 'Not supported.', 'it-l10n-ithemes-security-pro' )
			) );
		}
		...
		try {
			...
			return Result::error( new WP_Error(
				...
				__( 'Invalid lockout.', 'it-l10n-ithemes-security-pro' )
			) );
		}
		...
		if ( ! $lockout instanceof Lockout\Lockout ) {
			return Result::error( new WP_Error(
				...
				__( 'Lockout not found.', 'it-l10n-ithemes-security-pro' )
			) );
		}
		...
		if ( ! $lockout->get_host() ) {
			return Result::error( new WP_Error(
				...
				__( 'No host.', 'it-l10n-ithemes-security-pro' )
			) );
		}
	...
	public function execute_lock( Execute_Lock\Context $context ) {
		...
		if ( $context instanceof Execute_Lock\Host_Context && $context->is_network_brute_force() ) {
			...
			if ( ! $message ) {
				$message = __( 'Your IP address has been flagged as a threat by the Solid Security network.', 'it-l10n-ithemes-security-pro' );
			}
...
SQL,
	...
	public function notification_strings() {
		return array(
			'label'       => __( 'Site Lockouts', 'it-l10n-ithemes-security-pro' ),
			'description' => __( 'Various modules send emails to notify you when a user or IP address is locked out of your website.', 'it-l10n-ithemes-security-pro' ),
			'subject'     => __( 'Site Lockout Notification', 'it-l10n-ithemes-security-pro' ),
		);
	}
	...
	private function send_lockout_email( Lockout\Context $context, $host_expiration, $user_expiration, $reason ) {
		...
		$mail->add_header(
			esc_html__( 'Site Lockout Notification', 'it-l10n-ithemes-security-pro' ),
			esc_html__( 'Site Lockout Notification', 'it-l10n-ithemes-security-pro' ),
			false,
			sprintf( esc_html__( '%s lockout notification', 'it-l10n-ithemes-security-pro' ), $mail->get_display_url() ),
		);
		$mail->add_lockouts_table( $lockouts );

		if ( $show_remove_lockout_message ) {
			$mail->add_text( __( 'Release lockouts from the Active Lockouts section of the Security -> Dashboard page.', 'it-l10n-ithemes-security-pro' ) );
			$mail->add_button( __( 'Visit Dashboard', 'it-l10n-ithemes-security-pro' ), ITSEC_Mail::filter_admin_page_url( network_admin_url( 'admin.php?page=itsec-dashboard' ) ) );
		}

		if ( $show_remove_ip_ban_message ) {
			$mail->add_text( __( 'Release the permanently banned IP from the Banned IPs dashboard card.', 'it-l10n-ithemes-security-pro' ) );
			$mail->add_button( __( 'Visit Dashboard', 'it-l10n-ithemes-security-pro' ), ITSEC_Mail::filter_admin_page_url( network_admin_url( 'admin.php?page=itsec-dashboard' ) ) );
		}
	...
	public function filter_entry_for_list_display( $entry, $code, $data ) {
		...
		if ( 'whitelisted-host-triggered-blacklist' === $code ) {
			$entry['description'] = esc_html__( 'Authorized IP Triggered Ban Conditions', 'it-l10n-ithemes-security-pro' );
		} elseif ( 'host-triggered-blacklist' === $code ) {
			$entry['description'] = esc_html__( 'IP Triggered Ban Conditions', 'it-l10n-ithemes-security-pro' );
		} elseif ( 'whitelisted-host-triggered-host-lockout' === $code ) {
			$entry['description'] = esc_html__( 'Authorized IP Triggered IP Lockout', 'it-l10n-ithemes-security-pro' );
		} elseif ( 'host-lockout' === $code ) {
			if ( isset( $data[0] ) ) {
				$entry['description'] = sprintf( wp_kses( __( 'IP Lockout: <code>%s</code>', 'it-l10n-ithemes-security-pro' ), array( 'code' => array() ) ), $data[0] );
			} else {
				$entry['description'] = esc_html__( 'IP Lockout', 'it-l10n-ithemes-security-pro' );
			}
		} elseif ( 'whitelisted-host-triggered-user-lockout' === $code ) {
			$entry['description'] = esc_html__( 'Authorized IP Triggered User Lockout', 'it-l10n-ithemes-security-pro' );
			...
			$entry['description'] = esc_html__( 'Authorized IP Triggered Username Lockout', 'it-l10n-ithemes-security-pro' );
