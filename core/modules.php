...
final class ITSEC_Modules implements Import_Export_Source {
	...
	public static function register_module( $slug, $path ) {
		...
		if ( ! is_dir( $path ) ) {
			trigger_error( sprintf( __( 'An attempt to register the %1$s module failed since the supplied path (%2$s) is invalid. This could indicate an invalid modification or incomplete installation of the Solid Security plugin. Please reinstall the plugin and try again.', 'it-l10n-ithemes-security-pro' ), $slug, $path ) );

			return false;
		}

		if ( ! file_exists( $path . '/module.json' ) ) {
			trigger_error( sprintf( __( 'Registering a module without a module.json definition is invalid. Module: %s.', 'it-l10n-ithemes-security-pro' ), $slug ) );
		...
		if ( ! $json ) {
			trigger_error( sprintf( __( 'An attempt to register the %1$s module failed because it\'s configuration file is empty.', 'it-l10n-ithemes-security-pro' ), $slug ) );
		...
		if ( ITSEC_Core::is_development() && ( $valid = static::validate_module_config( $config ) ) !== true ) {
			trigger_error( wp_sprintf( __( 'An attempt to register the %1$s module failed because it has an invalid configuration: %2$l', 'it-l10n-ithemes-security-pro' ), $slug, ITSEC_Lib::get_error_strings( $valid ) ) );
		...
		if ( $extends = $config->get_extends() ) {
			...
			if ( ! $extends && ITSEC_Core::is_development() ) {
				trigger_error( wp_sprintf( __( 'An attempt to register the %1$s module failed because it extends a non-registered module: %2$l', 'it-l10n-ithemes-security-pro' ), $slug, $config->get_extends() ) );
		...
		if ( empty( $args['ignore_requirements'] ) ) {
			...
			if ( $validated->has_errors() ) {
				return new WP_Error(
					...
					__( 'Cannot activate module.', 'it-l10n-ithemes-security-pro' ) . ' ' .
	...
	public function get_export_title(): string {
		return __( 'Features', 'it-l10n-ithemes-security-pro' );
	}

	public function get_export_description(): string {
		return __( 'List of active modules.', 'it-l10n-ithemes-security-pro' );
	}
