...
<?php
	...
	private function show_sunset_banner( $packages ) {
		...
		if ( count( $sunset ) === 1 ) {
			$sunset_text = sprintf(
				__( '%s is being Sunset, but we have alternatives for you!', 'it-l10n-ithemes-security-pro' ),
				Ithemes_Updater_Functions::get_package_name( $sunset[0] )
			);
		} else {
			$sunset_text = __( 'iThemes plugins are being Sunset, but we have alternatives for you!', 'it-l10n-ithemes-security-pro' );
		}
			...
			<table class="ithemes-updater-listing widefat">
				<tbody>
					...
					<?php foreach ( $patchstack['sites'] as $site ): ?>
						<?php if ( ithemes_updater_site_has_patchstack( true, $site ) ): ?>
							<?php
							$class = 'active';
							if ( ++$count % 2 ) {
								$class .= ' alt';
