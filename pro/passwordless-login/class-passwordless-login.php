...
class ITSEC_Passwordless_Login {
	...
	private function register_profile_assets() {
		...
		if ( ! $response->is_error() ) {
			...
			wp_add_inline_script( 'itsec-passwordless-login-profile', sprintf(
				"wp.data.dispatch('%s').receiveUser( %s );", 'ithemes-security/core', wp_json_encode( rest_get_server()->response_to_data( $response, false ) )
			) );
		}
	...
	public function enqueue() {
		...
		if ( $this->should_include_ui( $action ) && empty( $_GET[ self::HIDE ] ) ) {
			...
			wp_enqueue_script( 'itsec-passwordless-login', plugin_dir_url( __FILE__ ) . 'js/login.js', $deps, 8 );
	...
	public function modify_login_errors( $error ) {
		...
		if ( ! empty( $_GET[ self::E_NOT_ALLOWED ] ) ) {
			$error->add(
				self::E_NOT_ALLOWED,
				__( 'Passwordless Login is not enabled for your account. Please log in with your username and password.', 'it-l10n-ithemes-security-pro' )
			);
		}
	...
	public function notification_strings() {
		return array(
			...
			'description' => sprintf(
				__( 'The %1$sPasswordless Login%2$s module sends an email with a link to automatically log in. Note: the default email template already includes the %3$s tag as a button.', 'it-l10n-ithemes-security-pro' ),
...
Click the button below to continue logging in.', 'it-l10n-ithemes-security-pro' ),
	...
	public function add_site_health_info( $info ) {
		...
		$query = new WP_User_Query( [
			...
			'meta_query' => [
				[
					'key'     => ITSEC_Passwordless_Login_Utilities::META_USES,
					'compare' => 'EXISTS',
				],
			]
		] );
		...
		$info['solid-security']['fields']['passwordless-users'] = [
			'label' => __( 'Passwordless Login Users', 'it-l10n-ithemes-security-pro' ),
			'value' => $total,
			'debug' => $total,
		];
